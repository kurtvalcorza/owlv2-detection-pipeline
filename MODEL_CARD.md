...
- **E2E Fine-tuning Tutorial**:
  ...
  *Text-prompted detection plus bounded class-head and box-head fine-tuning on a digest-pinned BCCD sample, with image-disjoint splits, per-phrase AP/mAP/precision/recall, two non-adapted baselines, SafeTensors adapter export, and fresh-base reload parity. The exact blob `c9cd132564ae` (commit `a772a31`) passed Kaggle Tesla T4 on 2026-09-25 UTC; see `docs/release-verification.md`.*
...
Operating environment: Python 3.12 with the exact pins in `pyproject.toml`, float32 model and adapter tensors, and CUDA selected automatically when visible. The inference-only carrier previously ran on CPU; the E2E default path was measured once on a Kaggle Tesla T4 (2026-09-25 UTC, 705.1 s wall including installs, one restart and downloads; bounded adaptation 297.0 s of which 244.2 s built the frozen-tower cache; frozen test evaluation 42.9 s and adapted test evaluation 54.2 s for 64 records). The frozen-tower cache stores about 1.7 GB of half-precision host tensors for the 300 train/validation records. Data environment: the default adaptation sample is BCCD blood-smear microscopy, which is a severe shift from the web photographs and detection corpora used upstream. Any measured result applies only to the three sample phrases and one seeded image-disjoint split; real cameras, other microscopes, other cell preparations, other phrase vocabularies, documents, aerial imagery, and attribute or relation phrases require separate measurement.
...
- E2E execution (exact blob `c9cd132564ae`, commit `a772a31`): Kaggle Tesla T4 (`kurtvalcorza/dimer-nb2-owlv2-detection` v2), 2026-09-25 UTC, torch 2.14.0+cu130 / transformers 4.57.6, `cuda:0`, float32; 705.1 s wall, 11/11 post-restart code cells. Default BCCD sample, one seeded image-disjoint split of 260 / 40 / 64 records; heads-only adaptation (1,579,526 of 154,966,792 parameters trainable), 8 epochs, best epoch 8 by validation mAP50 (0.8734 on 40 records). Held-out test (64 records, 846 reference boxes, score threshold 0.1, IoU 0.5): mAP50 empty 0.0 / grid prior 0.0147 / frozen 0.0723 / adapted 0.8918; precision 0.0 / 0.0146 / 0.1295 / 0.2236; recall 0.0 / 0.2967 / 0.0591 / 0.9894; F1 0.0 / 0.0279 / 0.0812 / 0.3647. Per-phrase AP frozen → adapted: platelet 0.0 → 0.8805 (50 reference boxes), red blood cell 0.0 → 0.8476 (731), white blood cell 0.2168 → 0.9473 (65). The adapted model emits 3,744 boxes for 846 references (837 matched), so its precision at 0.1 stays low without NMS or a tuned threshold. Adapter reload into a fresh verified base: 8/8 identical detections. One split, one run, no dispersion estimate; not a benchmark. Record: `docs/release-verification.md`.
- Not executed: BYOD, image-guided detection, the fast image processor, phrases near the 16-token limit, other microscopes or cell preparations, and any runtime other than the recorded Kaggle Tesla T4.

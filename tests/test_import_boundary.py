...
from owlv2_detection_pipeline.pipeline import (
    MANIFEST_NAME,
    MODEL_ID,
    MODEL_REVISION,
...
_CONFIG = json.dumps({"model_type": "owlv2", "projection_dim": 512}).encode()
...
def test_from_pretrained_refuses_without_snapshot_before_model_imports(tmp_path, forbid_model_imports):
    with pytest.raises(FileNotFoundError, match="allow_download=False"):
        Owlv2DetectionPipeline.from_pretrained(device="cpu", weights_dir=tmp_path, allow_download=False)
